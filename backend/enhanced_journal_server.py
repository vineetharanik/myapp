...
def enhanced_analyze_journal(entry: JournalEntry):
    ...
    try:
        ...
        existing_day = next((day for day in daily_stats if day.date == today), None)
        
        if existing_day:
            existing_day.total_study_hours += entry.study_hours or 0
            existing_day.dsa_problems_solved += entry.dsa_problems_solved
            existing_day.journal_entries += 1
        else:
            daily_stats.append(DailyStats(
                date=today,
                total_study_hours=entry.study_hours or 0,
                dsa_problems_solved=entry.dsa_problems_solved,
                journal_entries=1,
                average_mood=entry.mood_after or "neutral",
...
@app.get("/journal/daily-stats", response_model=List[DailyStats])
def get_daily_stats():
...
def get_weekly_summary():
    ...
    week_stats = {
        "total_study_hours": sum(day.total_study_hours for day in daily_stats[-7:]),
        "total_dsa_problems": sum(day.dsa_problems_solved for day in daily_stats[-7:]),
        "total_journal_entries": sum(day.journal_entries for day in daily_stats[-7:]),
        "average_burnout_risk": sum(day.burnout_risk for day in daily_stats[-7:]) / len(daily_stats[-7:]),
        "average_productivity": sum(day.productivity_score for day in daily_stats[-7:]) / len(daily_stats[-7:])
    }
